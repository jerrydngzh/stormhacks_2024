    ...
    const scrollChange = (top: boolean, change:number) => {
        if(top){
            ...
            if(change > 0){
                ...
                idx = idx == 0 ? props.bottoms.length - 1 : idx+change
            }
            updateBottom(idx)
        }
    }
        ...
        <>
            <div id="genView" className='container secondaryBG'>
                <div id="topClothing">
                    ...
                    <img id='topDisplay' className='entry' src={props.tops[topID].pictureLink}/>
                    <button id="upperR" className='rightBtn scrollBtn' onClick={() => scrollChange(true, 1)}>{"►"}</button>
                </div>
                <div id="lowerClothing">
                    <button id="lowerL" className='leftBtn scrollBtn btnBot' onClick={() => scrollChange(false, -1)}>{"◄"}</button>
                    <img id='bottomDisplay' className='entry' src={props.bottoms[bottomID].pictureLink}/>
                    <button id="lowerR" className='rightBtn scrollBtn btnBot' onClick={() => scrollChange(false, 1)}>{"►"}</button>
                </div>
            </div>
        </>
    )
